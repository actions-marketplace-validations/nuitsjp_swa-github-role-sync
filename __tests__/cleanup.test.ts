...
describe('cleanup action', () => {
  ...
  test('should delete expired discussions', async () => {
    ...
    expect(infoMock).toHaveBeenCalledWith(
      expect.stringContaining('Found category ID: cat1')
    )
    expect(infoMock).toHaveBeenCalledWith(
      expect.stringContaining('Deleting expired discussion')
    )
  ...
  test('should fail if category not found', async () => {
    ...
    expect(setFailedMock).toHaveBeenCalledWith(
      expect.stringContaining('Category "MissingCategory" not found')
    )
  })

  test('handles non-Error failures during cleanup', async () => {
    graphqlMock.mockRejectedValueOnce('non-error failure')

    const { run } = await loadCleanup()
    await run()

    expect(setFailedMock).toHaveBeenCalledWith('non-error failure')
  })
